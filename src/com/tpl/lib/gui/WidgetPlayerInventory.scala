...
class WidgetPlayerInventory(x: Int, y: Int, texture: Texture, invColor: Color = Color.white, hotbarColor: Color = Color.white) extends WidgetSubcontainer(new Rect(x,y,(3*18)+4+18,9*18)) with WidgetContainer {
  (0 to 2).foreach(row => {
    (0 to 8).foreach(col => {
